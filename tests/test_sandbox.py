...
def test_snapshot_store_record_and_list():
    ...
    snapshot = store.record(url="https://example.com", content="Example content", metadata={"a": 1})
...
def test_snapshot_store_diff():
    ...
    second = store.record(url="https://example.com", content="line1\nline2", metadata={})
...
def test_global_snapshot_store_prunes():
    ...
    for i in range(205):
        snapshot_store.record(url=f"https://example.com/{i}", content=f"content {i}", metadata={})
