...
from mcp_duckduckgo.models import DetailedResult, SearchResponse, SearchResult
...
def url_strategy(draw):
    ...
    domain = draw(
        st.text(
            alphabet=st.characters(whitelist_categories=["Ll"]), min_size=3, max_size=20
        )
    )
    tld = draw(st.sampled_from(["com", "org", "net", "edu", "gov"]))
    path = draw(
        st.text(
            alphabet=st.characters(whitelist_categories=["Ll", "Nd", "Pd"]), max_size=50
        )
    )
...
class TestPropertyBased:
    ...
    def test_search_response_invariants(self, results, total_results, page):
        ...
        response = SearchResponse(
            results=results, total_results=total_results, page=page
        )
    ...
    def test_detailed_result_creation(self, title: str, url: str, description: str):
        ...
        result = DetailedResult(title=title, url=url, description=description)
