...
from mcp_duckduckgo.enrichment import _normalize_entities, build_knowledge_graph
...
async def test_build_knowledge_graph_calls_link_entities():
    with patch(
        "mcp_duckduckgo.enrichment.link_entities", AsyncMock(return_value="graph")
    ) as link_mock:
...
async def test_build_knowledge_graph_handles_failures(caplog):
    with patch(
        "mcp_duckduckgo.enrichment.link_entities",
        AsyncMock(side_effect=RuntimeError("boom")),
    ):
        graph = await build_knowledge_graph(["Entity"], domain="domain")
    assert graph is None
    assert any(
        "Failed to construct knowledge graph" in message
